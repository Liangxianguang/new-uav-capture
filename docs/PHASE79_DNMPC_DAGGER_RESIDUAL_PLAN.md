...
1. 轮次 0：收集至少 `192` 条 DN-MPC teacher demonstrations。DN-MPC 每 16 步重新
   规划，使用 2 个 public-belief candidates 和 1 次局部 best-response 迭代；队列和延迟信息进入 public planning observation；每步保存 base action、
...
3. DAgger recovery round 1--2：用当前 actor 闭环访问 Nominal 状态，同时计算
   DN-MPC teacher 标签；当 teacher/base 差距、CBF 修正或 planner fallback 超过阈值
