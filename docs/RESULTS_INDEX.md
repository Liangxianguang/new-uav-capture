...
| Phase 26 | QDR delayed-state safety projection pilot | Evaluating local CBF at the first controllable delayed state improves the logged nominal prefix minimum clearance but leaves the paired 20-scene safe capture at 80.0% with 20.0% collision and raises total p95 to 149.90 ms; retain as a contract diagnostic, not a promoted gain. | `PHASE26_QDR_SAFETY_PROJECTION_PILOT_REPORT.md`, `tests/test_phase17_qdr_safety_projection.py` |
