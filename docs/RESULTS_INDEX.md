...
| Phase 26 | QDR delayed-state safety projection and prefix-risk audit | Evaluating local CBF at the first controllable delayed state improves the logged nominal prefix minimum clearance but leaves the paired 20-scene safe capture at 80.0% with 20.0% collision and raises total p95 to 149.90 ms; the released prefix audit finds 92.47% admissible QDR control steps and classifies the remaining first causes as obstacle/inter-agent/boundary. Retain as a contract diagnostic, not a promoted gain. | `PHASE26_QDR_SAFETY_PROJECTION_PILOT_REPORT.md`, `scripts/analyze_qdr_prefix_failures.py`, `tests/test_phase17_qdr_safety_projection.py` |
