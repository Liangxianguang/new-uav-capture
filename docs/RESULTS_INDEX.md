...
| Phase 17 | Queue-aware delayed-state rollout, uncertainty-triggered adaptive K/replanning, and reachability-normalized interception | QDR, UAKR, and RNIC adapters plus TensorBoard pilots implemented; QDR nominal-prefix geometry diagnostics are now auditable. All three remain validation-gated. Geometry-OOD Diagonal-SSM pilot shows UAKR compute savings but QDR remains a current No-Go for improvement claims. | `PHASE17_QUEUE_ADAPTIVE_REACHABILITY_TODOLIST.md`, `PHASE17_M0_QDR_SMOKE_REPORT.md`, `PHASE17_UAKR_RNIC_PILOT_REPORT.md`, `PHASE17_OOD_GEOMETRY_PILOT_REPORT.md` |
