...
| Phase 27 | RNIC formation-slot reachability cost | A bounded exact-permutation formation-slot objective is implemented and integrated into centralized and distributed team scoring. On one fixed 20-episode validation block it improves safe capture from 95.0% to 100.0% and mean clearance from 0.411 m to 0.448 m, but adds RNIC latency and is not yet a multi-seed promotion result. | `PHASE27_RNIC_FORMATION_SLOT_PILOT_REPORT.md`, `configs/phase27_rnic_formation_slot_pilot.yaml` |
