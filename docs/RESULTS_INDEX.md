...
| Phase 17 | Queue-aware delayed-state rollout, uncertainty-triggered adaptive K/replanning, and reachability-normalized interception | QDR, UAKR, and RNIC adapters plus TensorBoard pilots implemented; QDR nominal-prefix geometry diagnostics are auditable, but the 90-scene validation paired result is currently No-Go for QDR improvement. UAKR and RNIC remain validation-gated. | `PHASE17_QUEUE_ADAPTIVE_REACHABILITY_TODOLIST.md`, `PHASE17_M0_QDR_SMOKE_REPORT.md`, `PHASE17_QDR_VALIDATION_REPORT.md`, `PHASE17_UAKR_RNIC_PILOT_REPORT.md`, `PHASE17_OOD_GEOMETRY_PILOT_REPORT.md` |
