...
> 计算 checker，并统一输出 predictor/planner/QDR-or-tube/safety/total 的
> p50/p95/p99。Phase 59 的 GRU R2/R3 因实际 candidate count=1 不计入 K4/K8
> 结论；已补 candidate-budget audit，并用真正支持 sample_set 的 Diagonal-SSM
...
> Phase 63 QueueToken/QDR fresh calibration 已完成：全新矩阵共 `360 episodes /
> 180 mirror groups`，development-calibration 使用 `120 episodes / 60 groups`、
> 三个 Diagonal-SSM seeds。immutable QDR safe capture 为 `87.78%`、timeout
> `11.94%`、最大 exhaustion streak `113`；token-matched bounded replacement
> safe capture 降至 `74.17%`、timeout 升至 `24.72%`、streak `192`。相对 strong
> delayed-MPC 的安全轴改善存在，但 liveness gate 全部失败；QueueToken freshness
> accepted rate 为 `100%`，仍不构成安全证明。该 recovery authority 冻结为 No-Go，
...
> 审计和独立 QDR 时间索引审计均通过；`q={0,2,4,8,10}` 的正常 rollout 误差为
> 0，五个 double-delay negative probe 均被拒绝。候选预算日志显示 M8 为真实
> `K=8`，M0--M7 为 `K=1`，所以不能把 M6/M7 写成 K4/K8 或 adaptive-K 结果。
> M6/M7 在 delay-noise、joint-stress 中降低 collision，但 timeout 分别达到
> `12.22%/16.67%` 和 `33.33%/36.67%`，最大 exhaustion streak 为 `168/112`，
> 超过 24 步门槛；Phase 60 promotion No-Go，不进入 confirmation 或 locked-test。
> 完整 predictor/planner/QDR/safety/total 的 p50/p95/p99 见
> `docs/PHASE60_QDR_REVALIDATION_CALIBRATION_REPORT.md`。local CBF 仍只称经验
> 过滤器，robust CBF-QP/R-CLBF-QP 不宣称安全证明。
...
- [x] 冻结新的 protocol/runtime contract、方法 ID、seed、预算、tube 估计规则、
  async 通信合同和 Go/No-Go gate；不得先看结果再改配置；配置见
  `configs/phase58_delayed_planning_protocol.yaml` 和
  `configs/phase58_runtime_contract.yaml`；
- [x] 生成全新的 `480 episodes / 240 mirror groups`（最低 360/180），划分为
  ...
  manifest hash 为 `f4787252c2ca18e99316f0630f15c31c610504b60bba818b301b1cbf6af4207f`；
- [x] 实现 M0--M8 的 evaluator contract：current-state/known-delay delayed-MPC、
  fixed-tube/queue-aware-tube MPC、同步/异步 distributed MPC、QDR 组合和
  fixed-K=8 control；M0/M1/M6 已完成 smoke，oracle communication 保留为附录诊断；
