...
> P14 最新 authority matrix 已完成：hard `immutable` / `replace_nonexecuting` / `flush_pending` safe capture 为 `12.5% / 0% / 25.0%`，actual post-state safety 为 `29.55% / 90.05% / 99.75%`，均为 `0%` collision/boundary；结果仍是执行审计，不改变完整方法 No-Go 判定。详见 `docs/PHASE14_AUTHORITY_MATRIX_REPORT.md`。
...
当前审计已经证明静态 margin 不能覆盖延迟、tracking 和实际 post-step 状态。execution-state contract 已实现，但 queued command authority、恢复动作和求解器效率仍不足。恢复路线如下：
...
- [x] 修复 execution fallback 的零约束行缺陷，使用 analytic barrier-direction recovery；增加 recovery/zero/nominal 候选的独立 nonlinear certificate 和 TensorBoard `fallback_certificate_valid_rate`，固定矩阵结果仍为 No-Go，详见 `docs/PHASE5_RECOVERY_FALLBACK_AUDIT_REPORT.md`。
- [x] 增加 queue-aware recoverability contract：对当前不可修改的 queue prefix 给出 `abort_required`/`prefix_admissible`、prefix horizon 和 robust barrier 记录，并完成四 variant 固定矩阵及 TensorBoard/JSONL 保留；结果仍为 No-Go，详见 `docs/PHASE5_RECOVERABILITY_CONTRACT_AUDIT_REPORT.md`。
